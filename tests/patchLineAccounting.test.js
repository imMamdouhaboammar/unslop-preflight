...
test('safe repair validates actual changed lines instead of total file length', () => {
  ...
  const file = join(root, 'LongButtons.jsx');
  const filler = Array.from({ length: 80 }, (_, index) => `const filler${index} = ${index};`);
  const typelessButton = '<button>Open panel</button>';
  writeFileSync(file, [...filler, typelessButton].join('\n'), 'utf8');

  const result = runSourceFixEngine(root, [
    {
      rule: 'missing-button-type',
      file: 'LongButtons.jsx',
      excerpt: typelessButton
    }
  ], {
    safeFix: true,
    maxLinesPerFile: 4,
  ...
  assert.match(readFileSync(file, 'utf8'), /type="button"/);
});
