...
const NO_IMPECCABLE = flag('no-unslop');
...
function footer(allPassed) {
  ...
  if (allPassed && state.sourceFixes.length === 0 && !DRY_RUN) {
    ...
    console.log(`  ${c.dim}Run ${c.reset}${c.cyan}npx unslop shape${c.reset}${c.dim} to start building.${c.reset}`);
  }
...
async function runAutopilot() {
  ...
  if (!NO_IMPECCABLE) {
    process.stdout.write(`  ${c.dim}[setup]${c.reset}  Unslop setup gate                 `);
    if (DRY_RUN) phaseSkip('dry-run');
    else {
      const r = runNpx('unslop', ['skills', 'install']);
